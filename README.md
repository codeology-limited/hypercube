...
2. **Add a compartment**
   ...
   ```
   Optional knobs: `--compression {zstd|lz4|brotli|none}`, `--shuffle`, `--aont`, `--hash {sha3|blake3|sha256}`, `--whitener`, `--block-size`, `--mac-bits`, `--dimension`, and `--seal` (fills the file with random chaff compartments whose secrets are discarded).
3. **Extract**
   ```bash
...
## File Layout
- Header (plain JSON, after 4-byte `VHC\x01` magic) states the global transform choices: block size, MAC bits, compression type, shuffle/AONT/whitener/hash algorithms, etc.
- Blocks live back-to-back after the header. Each block is `16B sequence || block_size bytes of transformed data || mac_bytes`. Default sizes: `block_size = 4096`, `mac_bits = 256` ⇒ `mac_bytes = 32`, so each stored block is 4144 bytes.
...
- **Segment + Fragment**  
  Breaks the stream into fixed-size blocks, then into smaller fragments (default 64B) so shuffle/AONT can mix byte-level patterns.
...
- **Whitening**  
  Keccak- or XOR-based keystream that gets XORed with every fragment. This erases visual structure and makes every block look like high-entropy noise regardless of source content.
...
2. **MAC size** – Keep the default 256-bit MAC; 128-bit is only for low-stakes archives. 512-bit adds storage overhead with no real benefit unless audit/compliance demands it.
3. **Shuffle/AONT/Whitener choices** – Defaults offer the highest diffusion. Only change them when interoperability with another build matters.
4. **Sealing** – Running `--seal` after the last sensitive write adds random “dummy” compartments. This is the easiest way to hide how many real compartments exist.
5. **Backups** – The container is just a file. Back it up like any other encrypted volume; nothing special is required, but keep secrets off-box.
6. **Verification** – `cargo test` runs an extensive suite covering every pipeline stage, and `hypercube stats` helps spot corruption (a block with low entropy likely indicates tampering).
...
- **Defaults**: compression=`zstd`, shuffle=`feistel`, whitener=`keccak`, AONT=`rivest`, MAC hash=`sha3`, block_size=`4096`, mac_bits=`256`, dimension=`128`, fragment_size=`64` (derived).
...
```bash
hypercube add --secret "$(openssl rand -hex 32)" \
  --block-size 16384 --compression brotli --hash sha256 --mac-bits 512 \
  --dimension 512 archive.tar vault.vhc
```
